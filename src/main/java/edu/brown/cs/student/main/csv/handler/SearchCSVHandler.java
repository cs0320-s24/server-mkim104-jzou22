...
public class SearchCSVHandler implements RequestHandler {
    ...
    // example query: http://localhost:3232/searchcsv?searchValue=Rhode%20Island&header=true&columnIdentifier=City/Townd&columnType=
    @Override
    public Object handle(Request request, Response response) throws Exception {
        ...
        if (!header.equalsIgnoreCase("true") && !header.equalsIgnoreCase("false") &&
                !columnIdentifier.equalsIgnoreCase("true") && !columnIdentifier.equalsIgnoreCase("false")) {
            response.status(400);
            return "[header] only takes in a 'true' or 'false'";
        }
        if (!columnType.equalsIgnoreCase("I") && !columnType.equalsIgnoreCase("N")
        ...
        CSVSearcher searcher = new CSVSearcher(this.loadCSVHandler.getData(),
                newSearchValue,
                newColumnIdentifier,
                headerExistence,
                columnType);
